...
# vocab().most_common를 이용햇 ㅓ가장 자주 사용되는 단어 빈도수 확인
...
# 한글폰트를 로드해야 깨지지 않고 출력됨
font_fname = 'C:\Windows\Fonts\gulim.ttc'
font_name = font_manager.FontProperties(fname=font_fname).get_name()
rc('font', family=font_name)

#plt.figure(figsize=(20,10)) # 도화지 그리기
#text.plot(50)
#plt.show()

# 자주 사용되는 토큰 5000개를 사용해서 데이터를 벡터화시킨다
...
train_x = [term_frequency(d) for d, _ in train_docs] # 앞에만 가져오기
test_x = [term_frequency(d) for d, _ in test_docs]
train_y = [c for _, c in train_docs] # 뒤에만 가져오기
test_y = [c for _, c in test_docs]

# 이제 데이터를 float로 형 변환시켜주면 데이터 전처리 과정 끝
print('>> 형변환 시작')
x_train= np.asarray(train_x).astype('float32') #np: 수치연산 쉽게 해줌.
...
# - 차례대로 층을 쌓아 만든 모델을 tf.keras.Sqquentila 모델

model = models.Sequential() # 모델 생성

# 모델 층을 구성
model.add(layers.Dense(64, activation='relu', input_shape=(5000,))) # 1층 생성(input층)
model.add(layers.Dense(64, activation='relu')) # 2층 생성
model.add(layers.Dense(1, activation='sigmoid')) # 3층 생성(output층)

# 모델 훈련 준비(훈련과정을 설정) : 어떻게 학습할지 방법 알려줌
...
model.compile(optimizer=optimizers.RMSprop(lr=0.001),
                ...
                metrics = [metrics.binary_accuracy]) # 손실에 따라 weight 설정

# 모델 훈련
model.fit(x_train, y_train, epochs=10, batch_size=512) # 어떻게 학습할것인지
# epochs = 10 -> 10번 반복해서 학습해라
# batch_size=512 -> 512개씩 학습해라
